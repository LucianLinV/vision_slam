...
double ComputeSo3RmsError(const neves::SO3d &R,
...
void PrintSo3Stats(const std::string &name, const So3Stats &stats,
                   int repeat_count) {
...
int main() {
  ...
  ceres_stats.rms_error =
      ComputeSo3RmsError(ceres_stats.estimated_R, points, observations);
  g2o_stats.angle_error = (g2o_stats.estimated_R.inverse() * true_R).log().norm();
  g2o_stats.rms_error = ComputeSo3RmsError(g2o_stats.estimated_R, points,
                                           observations);

  std::cout << "true so3:      " << true_phi.transpose() << '\n';
  std::cout << "point count:   " << points.size() << '\n';
  std::cout << "repeat count:  " << repeat_count << '\n';
  PrintSo3Stats("ceres", ceres_stats, repeat_count);
  PrintSo3Stats("g2o", g2o_stats, repeat_count);
