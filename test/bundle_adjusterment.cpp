...
neves::Vec2d Project(const cv::Mat &K, const g2o::SE3Quat &pose,
...
double PixelRmsError(const cv::Mat &K, const g2o::SE3Quat &pose,
...
double RotationErrorRad(const g2o::SE3Quat &estimated,
                        const g2o::SE3Quat &truth) {
  const Eigen::Matrix3d R_error =
      (estimated.inverse() * truth).rotation().toRotationMatrix();
...
int main() {
  ...
  const neves::Vec6d true_update =
      (neves::Vec6d() << 0.35, -0.18, 0.42, 0.08, -0.05, 0.12).finished();
  const g2o::SE3Quat true_pose = g2o::SE3Quat::exp(true_update);
  ...
  const neves::Vec6d init_update =
      (neves::Vec6d() << 0.10, 0.10, 0.20, -0.05, 0.03, -0.02).finished();
  const g2o::SE3Quat init_pose = g2o::SE3Quat::exp(init_update);

  const cv::Mat init_R =
      EigenRotationToCvMat(init_pose.rotation().toRotationMatrix());
  ...
  const neves::BundleAdjustmentResult result =
  ...
  const bool chi2_improved = result.final_chi2 < result.initial_chi2 * 0.05;
  ...
  return chi2_improved && pose_improved && fit_matches_noise ? 0 : 1;
}
